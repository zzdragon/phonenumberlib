...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private List<PyEntity> mData;
    private Paint mPaint;
    private Rect mBounds;

    private int mTitleHeight;
    private static int TITLE_BG_COLOR = Color.parseColor("#FFEFEFEF");
    private static int TITLE_TEXT_COLOR = Color.parseColor("#666666");
    private static int mTitleTextSize;


    public TitleItemDecoration(Context context, List<PyEntity> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getPy() && !mData.get(position)
                            .getPy().equals(mData.get(position - 1).getPy())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).getPy(), 0, mData.get(position).getPy().length(), mBounds);
        c.drawText(mData.get(position).getPy(),
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).getPy() &&
                        !mData.get(position).getPy().equals(mData.get(position - 1).getPy())) {
